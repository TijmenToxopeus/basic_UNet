...
The examples below show one representative case per dataset as animated 4-panel views:
`[Input | GT | Prediction | Overlay]`.

The overlay uses the same class colors as the analysis notebook:

- RV: orange
- MYO: green
- LV: blue

Ground truth is shown with semi-transparent fills and solid contours, predictions are shown with dashed contours, and disagreement regions are highlighted in red.
