...
def evaluate(cfg=None, debug=False):
    ...
    print(
        f"⏱️ Total evaluation time: {total_eval_time_s:.1f}s "
        f"({total_eval_time_s/60.0:.2f} min, {total_eval_time_s/3600.0:.2f} h)"
    )
    ...
    summary["eval"] = {
        ...
        "time": {
            "total_seconds": float(total_eval_time_s),
            "total_minutes": float(total_eval_time_s / 60.0),
            "total_hours": float(total_eval_time_s / 3600.0),
        },
