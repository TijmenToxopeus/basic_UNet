...
def evaluate(cfg=None, debug=False):
    ...
    if model_ckpt is None or not model_ckpt.exists():
    ...
    # ------------------------------------------------------------
    # Seeded selection of visualization indices
    # (use Python RNG seeded from experiment seed)
    # ------------------------------------------------------------
    rng = random.Random(seed)
    visual_indices = rng.sample(
    ...
    # Reset VRAM peak counter (CUDA only)
    if device.type == "cuda":
    ...
    metrics = {
        ...
        "per_class": {
            ...
            for name, dm, ds, im, is_ in zip(
                class_names, avg_class_dice, std_class_dice, avg_class_iou, std_class_iou
            )
        }
    }
