...
# Quant mapping / misc
# -----------------------
def _make_histogram_act_perchannel_wt_qconfig() -> torch.ao.quantization.QConfig:
    """
    HistogramObserver for activations (per-tensor affine) + per-channel weight fake quant.
    Matches the 'Oscar-style' idea while remaining backend-compatible.
    """
    # Activation fake quant with histogram observer
    ...
    # Per-channel weight fake quant (standard default used widely for conv weights)
    wt = torch.ao.quantization.default_fused_per_channel_wt_fake_quant
    return torch.ao.quantization.QConfig(activation=act, weight=wt)


def build_qconfig_mapping(
    backend: str,
    *,
    qconfig_mode: str = "default",  # <-- ADDED
    keep_convtranspose_fp32: bool,
    keep_final_conv_fp32: bool,
    keep_batchnorm_fp32: bool,
    keep_concat_fp32: bool,
    ...
    qconfig_mode:
      - "default": torch.ao.quantization.get_default_qconfig(backend)
      - "histogram": HistogramObserver activations + per-channel weight fake quant
    ...
    NOTE on keep_concat_fp32:
      ...
      simply behave as if keep_concat_fp32=False (still safe).
    """
    if qconfig_mode == "histogram":
        qconfig = _make_histogram_act_perchannel_wt_qconfig()
    else:
        qconfig = get_default_qconfig(backend)
