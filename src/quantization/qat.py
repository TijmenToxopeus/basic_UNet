...
def run_qat(
    ...
    *,
    ...
    if qconfig_mode not in {"default", "histogram"}:
        raise ValueError("qconfig_mode must be one of: default, histogram")
    ...
    # <-- MODIFIED: pass qconfig_mode through
    qconfig_mapping = build_qconfig_mapping(
        backend,
        qconfig_mode=qconfig_mode,
        keep_convtranspose_fp32=keep_convtranspose_fp32,
        keep_final_conv_fp32=keep_final_conv_fp32,
        keep_batchnorm_fp32=keep_batchnorm_fp32,
        keep_concat_fp32=keep_concat_fp32,
    ...
    paths = get_paths(cfg)
    q_dir = paths.base_dir / "quantization_qat" / f"{model_phase}_{backend}_{qconfig_mode}"
    q_dir.mkdir(parents=True, exist_ok=True)
    ...
    summary["quantization"] = {
        ...
        "model_phase": model_phase,
        "backend": backend,
        "qconfig_mode": qconfig_mode,  # <-- ADDED
        "model_checkpoint": str(model_ckpt),
        "qat_training": {
            "train_source": train_source,
            "img_dir": str(train_img_dir),
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="default",
        choices=["default", "histogram"],
        help="Quantization config mode. 'histogram' uses HistogramObserver for activations + per-channel weight fake-quant.",
    )
...
def main():
    ...
    summary_path = run_qat(
        ...
        train_source=args.train_source,
        backend=args.backend,
        qconfig_mode=args.qconfig_mode,  # <-- ADDED
        qat_epochs=args.qat_epochs,
        qat_lr=args.qat_lr,
        qat_grad_clip=grad_clip,
        bn_freeze_after=args.bn_freeze_after,
