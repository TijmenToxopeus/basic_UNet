...
# 2. SIMILARITY MATRIX (AVERAGED ACROSS SLICES)
############################################################
def compute_similarity_matrix_multi(
    ...
    if metric not in ("pearson", "cosine"):
        raise ValueError(f"Unknown similarity metric '{metric}'. Expected 'pearson' or 'cosine'.")
...
def get_redundancy_masks(
    ...
    *,
    ...
    if similarity_metric not in ("pearson", "cosine"):
        raise ValueError(
            f"Unknown similarity metric '{similarity_metric}'. Expected 'pearson' or 'cosine'."
        )

    metric_label = "pearson_correlation" if similarity_metric == "pearson" else "cosine_similarity"

    print(f"\n🔥 Running {metric_label}-based pruning (L1-compatible mode)...")
    print(f"  Using {len(example_slices)} slices")
    print(f"  Batch size     = {batch_size}")
    print(f"  threshold      = {threshold}")
    ...
    for block, layer_names in block_to_layers.items():
        ...
        if prune_ratio <= 0.0:
            ...
            sim = compute_similarity_matrix_multi(
                fmaps,
                device=device,
    ...
    if save_dir:
        ...
        artifact_prefix = "correlation" if similarity_metric == "pearson" else "cosine_similarity"
        torch.save(masks, save_path / f"{artifact_prefix}_masks.pt")
        meta = {
        ...
        (save_path / f"{artifact_prefix}_masks_meta.json").write_text(json.dumps(meta, indent=2))
